...
export default function Results() {
  ...
  const getResult = () => {
    if (Math.abs(currentPercentage - target) < 0.01) {
      ...
      return {
        message: `You must attend the next ${classesNeeded} ${
      ...
      return {
        message: `You can bunk the next ${classesCanbunk} ${
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#1A1D21]">
      <View className="flex-1 px-6 pt-12">
        ...
        {/* Summary Cards */}
        <View className="mb-8">
          <View className="bg-[#23282D] rounded-xl px-4 py-3 shadow-sm mb-3">
            <Text className="text-xs text-gray-500 uppercase mb-1">
              Classes Attended
            </Text>
            <Text className="text-lg font-semibold text-white">{present}</Text>
          </View>
          ...
          <View className="bg-[#23282D] rounded-xl px-4 py-3 shadow-sm mb-3">
            ...
            <Text className="text-lg font-semibold text-white">{target}%</Text>
          </View>

          <View className="bg-[#23282D] rounded-xl px-4 py-3 shadow-sm">
            <Text className="text-xs text-gray-500 uppercase mb-1">
              Current Percentage
            </Text>
        ...
        {/* Results Card */}
        <View className="bg-[#23282D] rounded-3xl p-6 mb-6 shadow-xl">
          <Text className="text-sm font-bold text-white text-center mb-4 uppercase tracking-wide">
            YOUR RESULTS:
          </Text>
